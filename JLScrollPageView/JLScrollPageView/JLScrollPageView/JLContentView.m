...
@interface JLContentView ()<UICollectionViewDelegate, UICollectionViewDataSource, UIScrollViewDelegate, UIGestureRecognizerDelegate> {
    NSInteger _oldIndex;
...
@property (strong, nonatomic) NSMutableDictionary<NSString *, UIViewController<JLScrollPageViewChildControllerDelegate> *> *childVcsDic;

/** 当前控制器 */
@property (strong, nonatomic) UIViewController<JLScrollPageViewChildControllerDelegate> *currentChildVc;

@end

@implementation JLContentView
...
- (void)receiveMemoryWarningHander:(NSNotificationCenter *)noti {
    ...
    [_childVcsDic enumerateKeysAndObjectsUsingBlock:^(NSString * _Nonnull key, UIViewController<JLScrollPageViewChildControllerDelegate> * _Nonnull childVc, BOOL * _Nonnull stop) {
        __strong typeof(self) strongSelf = weakSelf;

        if (childVc != strongSelf.currentChildVc) {
            [_childVcsDic removeObjectForKey:key];
            [JLContentView removeChildVc:childVc];
        }
...
- (void)reload {
    ...
    [self.childVcsDic enumerateKeysAndObjectsUsingBlock:^(NSString * _Nonnull key, UIViewController<JLScrollPageViewChildControllerDelegate> * _Nonnull childVc, BOOL * _Nonnull stop) {
        [JLContentView removeChildVc:childVc];
        childVc = nil;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(childViewController:forIndex:)]) {
        if (_currentChildVc == nil) {
            ...
            if (!_currentChildVc || ![_currentChildVc conformsToProtocol:@protocol(JLScrollPageViewChildControllerDelegate)]) {
                NSAssert(NO, @"子控制器必须遵守JLScrollPageViewChildControllerDelegate协议");
            }
...
- (NSMutableDictionary<NSString *,UIViewController<JLScrollPageViewChildControllerDelegate> *> *)childVcsDic {
    if (!_childVcsDic) {
